...
const create_form_validator = joi.object({
  ...
  phone_number: joi.string().trim().pattern(/^09\d{9}$/).required(),
  address: joi.string().trim().max(100).required(),
  postal_code: joi.string().trim().pattern(/^\d{10}$/).required(),
  ...
  owner_national_code: joi.string().trim().pattern(/^\d{10}$/).when("same_policyholder_owner", {
    is: "0",
    then: joi.required(),
    otherwise: joi.allow("", null),
  }),

owner_birthday_date: joi.string()
  .trim()
