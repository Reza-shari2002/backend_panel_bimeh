...
async function SendNotification_submit(data) {
    try{
        ...
        const response =  await SendNotificationApi([message , "پیام برای کاربر ارسال شد"]  , [data.phone_number , "09166082219"]);
        return response;
    }
    catch(err){
        throw err;
...
async function SendNotification_first_message(data) {
    try{
        ...
        const response =  await SendNotificationApi([message1,message2]  , [data.phone_number,'09166082219']);
        return response;
    }
    catch(err){
        throw err;
