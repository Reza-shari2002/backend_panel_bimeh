...
async function find_user_data(id) {
  try {
    const query = "select * from motor_third_party_insurance_forms where id = ?";
...
async function find_users_data() {
  try {
    const query =
      "select phone_number, payment_type, id, address, postal_code, plate_history_code, has_discount_transfer, has_active_insurance_transfer, is_relative_transfer, plate_history_type, document_car_type, full_name, created_at from motor_third_party_insurance_forms order by created_at desc";
...
async function insertForm(files, body) {
  ...
  try {
    const query = "INSERT INTO motor_third_party_insurance_forms SET ?";
...
async function find_records_number() {
  try {
    const query = 'SELECT COUNT(*) AS total FROM motor_third_party_insurance_forms';
...
async function  finde_query_record(limit , offset) {
  ...
  FROM motor_third_party_insurance_forms 
